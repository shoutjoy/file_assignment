...
### 경기대학교 — 파일 제출과 온라인 글 통합

통합 탭의 **다운로드 형식**에서 **경기대**를 선택하면 `이름_학번_assignsubmission_file_` 및 `이름_학번_assignsubmission_onlinetext_` 폴더를 인식합니다. 두 유형의 폴더가 함께 있어도 됩니다.

- `assignsubmission_file_` 폴더의 파일은 하위 폴더까지 찾아 **저장할 폴더 한곳**으로 복사합니다. 기존 파일과 이름이 겹치면 번호를 붙입니다. 파일명에 이름·학번을 붙이는 옵션도 적용됩니다.
- `assignsubmission_onlinetext_` 폴더의 `.txt`, `.text`, `.md`, `.markdown`, `.html`, `.htm` 파일은 **`온라인텍스트_통합.md`** 한 문서로 합칩니다. 학생별로 `## 이름_학번` 제목 아래 글을 넣고, 학생 이름·학번 순서로 정렬합니다. HTML은 태그를 제거해 본문 텍스트를 넣습니다.
- 학생 명단 CSV 옵션을 켜면 같은 학생의 두 제출 유형은 명단에서 한 줄로 합쳐집니다.

예를 들어 `송민정_1699618_assignsubmission_onlinetext_`의 글과 `김지혜_1699620_assignsubmission_file_`의 첨부파일을 같은 저장 폴더로 통합할 수 있습니다.
...
1. 프로그램을 실행합니다.  
2. **다운로드 형식**에서 연세대 또는 경기대를 선택합니다.  
3. **대상 폴더 (주차별 과제)**  
...
5. **실행 옵션**  
   ...
   - 통합 완료 후 프로그램 종료  
6. **통합 작업 시작** 을 누릅니다.  
7. **진행 현황** 에서 복사·건너뜀·CSV 생성 결과를 확인합니다.  
8. 완료 메시지 후 저장 폴더에서 통합 파일과 `학생명단_리스트.csv` 를 확인합니다. 경기대 온라인 글이 있으면 `온라인텍스트_통합.md`도 생성됩니다.
...
| `gyeonggi_submission.py` | 경기대 제출 폴더 인식, 온라인 글·첨부파일 통합 |
